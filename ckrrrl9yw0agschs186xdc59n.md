...
- `git branch -a`: Run this command to see all the branches in your repo **(local + remote)** . The branches shown in the form `<branch name>` is/ are the **local branche(s)** in the local repo. Similarly, the branches shown in the form `remotes/origin/<branch name>` is/ are the **remote branch(s)** from the remote repo **origin** connected to the local repo. 
